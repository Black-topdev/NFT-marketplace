...
function TncDapp() {
    ...
    this.populateMyErc1155s = async function (){
        ...
        let length = await window.tncLib.getMyHrc1155Length();

        let offset = _this.lastHrc1155Index > -1 ? _this.lastHc1155Index : length;
        ...
        for(let i = offset - 1; i >= 0; i--){
            currentIndex = i;
            let pool = await window.tncLib.getMyHrc1155(i);
            let _uri = pool.contractURI;

            try {

                console.log("CALLING FETCH URL: ", pool.erc1155);
                fetchUrl(api_url + '1.0/'+chain_id+'/collections/events/URI/hrc1155Address/'+pool.hrc1155+'/id/0', 5000);
                ...
                if (typeof data == 'object') {
                    ...
                    $('.manageNftsButton' + i).on('click', function () {
                        ...
                        $('#nftHrc1155Address').val($(this).data('contractAddress'));
                        $('#nftHrc1155CurrName').val($(this).data('contractName'));
    ...
    this.populateMyNfts = async function(){
        ...
        let erc1155Address = $('#nftHrc1155Address').val();
        ...
        if(offset <= 0) {
            ...
            $('#nftContractNameTitle').text($('#nftHrc1155CurrName').val());
        }
        ...
        for(let i = offset; i < nfts.length; i++){
            ...
            let nft = await window.tncLib.getNft(hrc1155Address, nfts[i]);
    ...
    this.newUpdateCollection = async function(e){
        ...
        if( $(e.relatedTarget).data('edit') ){
            ...
            let contractMeta = await window.tncLib.getHrc1155Meta($(e.relatedTarget).data('contractAddress'));
            ...
            try{
                ...
                if (typeof data == 'object') {
                    ...
                    $('#hrc1155Submit').html('Update Collection');
                    $('#hrc1155Description').val( data.description );
                    $('#hrc1155ExternalUrl').val( data.external_link );
                    $('#hrc1155ImageUrl').val( data.image );
                    if(data.image.trim() != "") {
                        $('.imageFileDisplay').html('<img src=' + JSON.stringify(data.image) + ' border="0" width="200"/>');
                    }
                    $('#hrc1155Name').val( data.name );
                }
                else{
    ...
    this.processTransferMultiBatch = async function(){
        ...
        if(!await tncLib.isHrc1155Supported(collection)){
        ...
        let approved = await tncLib.hrc1155IsApprovedForAll(tncLib.account, tncLib.multiBatch.options.address, collection);
     ...
     */
    document.getElementById('hrc1155ImageFile').onchange = function () {

        _this.storeIpfsImage( document.getElementById('hrc1155ImageFile'), document.getElementById('hrc1155ImageUrl') );
    };
    ...
    $('#hrc1155Submit').on('click', async function(){

        if(
            (
                $('#collectionIsEdit').val() == '0' && $('#hrc1155Ticker').val().trim() == ''
            ) ||
            $('#hrc1155Name').val().trim() == ''
        ){
            _alert('Please enter all required fields.');
            return;
        }

        let name = $('#hrc1155Name').val();
        let ticker = $('#hrc1155Ticker').val();
        let description = $('#hrc1155Description').val();
        let imageUrl = $('#hrc1155ImageUrl').val();
        let externalUrl = $('#hrc1155ExternalUrl').val();
